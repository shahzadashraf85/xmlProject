...
const CP_API_URL = "https://soa-gw.canadapost.ca/rs"; // Use /rs for production, /vis/rs for dev
// const CP_API_URL = "https://ct.soa-gw.canadapost.ca/rs"; // Development URL
...
<shipment xmlns="http://www.canadapost.ca/ws/shipment-v8">
  <group-id>${order.id.substring(0, 32)}</group-id>
  ...
  <delivery-spec>
    ...
    <destination>
      ...
      <address-details>
        ...
        <postal-zip-code>${order.postal_code.replace(/\s/g, '').toUpperCase()}</postal-zip-code>
      </address-details>
    </destination>
    ...
    <references>
      ...
      <customer-ref-1>${order.reference_number || order.id}</customer-ref-1>
    </references>
  </delivery-spec>
</shipment>`

        // 3. Call Canada Post API
        const response = await fetch(`${CP_API_URL}/${CP_CUST_NUM}/${CP_CUST_NUM}/shipment`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            console.error("CP Error:", responseText);
            // Simple XML Parsing to find error message
            const match = responseText.match(/<description>(.*?)<\/description>/);
            const errorMsg = match ? match[1] : response.statusText;
